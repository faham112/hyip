 ...
 */
function hyip_process_daily_profits($investment_id = null) {
    global $wpdb;
    ...
    $where_clause = "WHERE i.status = 'active' AND (i.next_profit_date IS NULL OR i.next_profit_date <= NOW()) AND i.days_completed < p.duration";
    $params = array();
    
    if ($investment_id) {
        $where_clause .= " AND i.id = %d";
        $params[] = $investment_id;
    }
    
    // Get investments due for profit
    $investments = $wpdb->get_results($wpdb->prepare("
        SELECT i.*, p.name as plan_name, p.duration, p.daily_percent 
    ...
    foreach ($investments as $investment) {
        // Start transaction for each investment
        $wpdb->query('START TRANSACTION');
        ...
        try {
            ...
            $user_update = $wpdb->query($wpdb->prepare("
                UPDATE $table_users 
            ...
            $new_total_profit = $investment->total_profit + $daily_profit;
            $status = ($new_days_completed >= $investment->duration) ? 'completed' : 'active';
            $completion_date = ($status == 'completed') ? current_time('mysql') : null;
            $next_profit_date = ($status == 'active') ? 
                hyip_get_next_profit_date($investment->invest_date, $new_days_completed) : null;
            
            $investment_update = $wpdb->update(
                $table_investments,
                array(
                    'total_profit' => $new_total_profit,
                    'days_completed' => $new_days_completed,
                    'next_profit_date' => $next_profit_date,
                    'status' => $status,
            ...
            if (!$investment_update) {
                throw new Exception('Failed to update investment');
            }
            ...
            if (!$transaction_insert) {
                throw new Exception('Failed to log earning transaction');
            }
            
            // If investment completed, return principal
            if ($status == 'completed') {
                // Update user balance with principal return
                ...
                $wpdb->insert(
                    ...
                    array(
                        ...
                        'description' => 'Principal return from ' . $investment->plan_name,
                        'reference_id' => $investment->id,
                        'status' => 'completed',
                        'transaction_date' => current_time('mysql')
                    )
                ...
                hyip_send_investment_completion_notification($investment->user_id, $investment);
            }
            ...
            // Rollback transaction
            $wpdb->query('ROLLBACK');
            error_log('HYIP: Failed to process profit for investment ' . $investment->id . ': ' . $e->getMessage());
        }
    }
